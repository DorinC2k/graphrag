...
| `GRAPHRAG_LLM_TEMPERATURE`                        |                          | The temperature to use generation. For Responses API models (gpt-4.1*, gpt-5*), a configured value of `0` is ignored so the OpenAI default (`1`) is used.
                                | `float` | 0                     |
...
| `HUGGINGFACEHUB_API_TOKEN`                              | **For HuggingFace**      | Alternative name for the Hugging Face API token.                                             | `str`   | `None`                   |
| `HUGGINGFACE_API_BASE`                                  | **For HuggingFace**      | The API base URL for a Hugging Face Inference Endpoint.                                      | `str`   | `None`                   |
| `GRAPHRAG_EMBEDDING_TYPE`                               | **For AOAI**             | The embedding client to use. Either `huggingface_embedding`, `openai_embedding`, or `azure_openai_embedding`                                         | `str`   | `huggingface_embedding`       |
...
| `GRAPHRAG_EMBEDDING_MODEL`                              |                          | The model to use for the embedding client.                                                                                 | `str`   | `sentence-transformers/all-MiniLM-L6-v2` |
